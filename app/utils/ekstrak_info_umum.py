...
def ekstrak_info_umum(entities_input_list):
    ...
    for i, entity in enumerate(entities_sorted):
        ...
        if group == 'ORG':
            ...
            entitas_diproses_untuk_info_umum = True
        elif group == 'DOCDATE' and start_pos < 200:  # Asumsi tgl surat di awal
            info_umum_dokumen["tanggal_surat_dokumen"].append(word)
            entitas_diproses_untuk_info_umum = True
        elif group == 'CITY' and start_pos < 200:  # Asumsi kota surat di awal
    ...
    for key, val_list in info_umum_dokumen.items():
        ...
        if key == "nomor_surat" and unique_ordered:
            # Logika khusus untuk menggabungkan bagian-bagian nomor surat
            if len(unique_ordered) == 1:
                full_doc_num = unique_ordered[0]
            else:
                # Gabungkan bagian-bagian, pastikan satu '/' sebagai pemisah
                # dan bersihkan spasi di dalam setiap segmen
                processed_parts = []
                for k_part_idx, part_str in enumerate(unique_ordered):
                    # Bersihkan spasi internal di sekitar '/' dalam satu part dulu
                    # e.g., "b/hut /omk d / vii / 2024" -> "b/hut/omk d/vii/2024"
                    ...
                    if k_part_idx == 0:
                        ...
                        if prev_part.endswith('/'):
                            if cleaned_inner_part.startswith('/'):
                                processed_parts.append(cleaned_inner_part[1:])  # Hapus '/' depan dari part saat ini
                            else:
                                processed_parts.append(cleaned_inner_part)
                        elif not cleaned_inner_part.startswith('/'):
                            processed_parts.append('/')  # Tambah pemisah
                            processed_parts.append(cleaned_inner_part)
                        else:  # prev_part tidak diakhiri '/' TAPI cleaned_inner_part diawali '/'
                            processed_parts.append(cleaned_inner_part)
                full_doc_num = "".join(processed_parts)

            # Pembersihan akhir untuk spasi di sekitar '/' dan duplikasi '/'
            full_doc_num = re.sub(r'\s*/\s*', '/', full_doc_num)
            full_doc_num = re.sub(r'/+', '/', full_doc_num)
            info_umum_dokumen[key] = full_doc_num
        elif key == "penerima_surat": continue
        elif key == "organisasi":
            info_umum_dokumen[key] = [{'nama': org} for org in unique_ordered] if unique_ordered else []
        elif unique_ordered:  # Untuk field info umum lainnya

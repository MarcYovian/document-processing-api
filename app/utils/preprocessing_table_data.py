...
def extract_table_grid_from_page(binary_page_for_lines, id_numerik, page_number):
    ...
    if binary_page_for_lines is None:
        print(f"Tidak ada gambar biner untuk ekstraksi grid di Halaman {page_number + 1}.")
        return None
    ...
    if np.sum(table_grid_mask) == 0: # Jika semua piksel hitam (tidak ada garis putih terdeteksi)
        print(f"Tidak ada grid tabel (garis) yang terdeteksi dengan jelas di Halaman {page_number + 1}.")
        return None

    print(f"--- Berhasil Ekstraksi Grid Tabel untuk Halaman {page_number+1} ---")
...
def page_after_line_removal(binary_page_with_lines, table_grid_mask, id_numerik, page_number):
    ...
    if binary_page_with_lines is None:
        print(f"Tidak ada gambar biner input untuk Halaman {page_number + 1}.")
        return None
    if table_grid_mask is None:
        print(f"Tidak ada masker grid tabel untuk Halaman {page_number + 1}, menggunakan gambar biner asli.")
    ...
    print(f"Gambar setelah penghapusan garis disimpan sebagai: {debug_path_lines_removed}")
